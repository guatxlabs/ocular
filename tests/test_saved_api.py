...
def test_lookup_saved_url_422_on_malformed_url_not_500(tmp_path, monkeypatch):
    # Une URL malformée (normalize_url lève ValueError) doit
...
def test_verdict_by_non_owner_is_404_and_owner_can(tmp_path, monkeypatch):
    # Isolation par propriétaire (MEDIUM) : alice sauvegarde ; bob (autre
    # identité forward-auth du MÊME déploiement) ne peut PAS écraser le verdict
    # d'alice -> 404 (même réponse que « inconnue » : pas d'oracle). La
    # propriétaire alice, elle, le peut.
    c, q, tp = _client_forward_auth(tmp_path, monkeypatch, "alice")
    _seed_job(q, tp)
    sid = c.post("/saved", json={"job_id": "jx"}).json()["id"]

    cb = TestClient(app)
    cb.headers.update({"X-Forwarded-User": "bob"})
    r = cb.post(f"/saved/{sid}/verdict", json={"analyst_verdict": "legitimate"})
    assert r.status_code == 404
    # le verdict de bob n'a PAS été posé
    entry = next(x for x in c.get("/saved").json() if x["id"] == sid)
    assert entry["analyst_verdict"] is None

    ra = c.post(f"/saved/{sid}/verdict", json={"analyst_verdict": "malicious"})
    assert ra.status_code == 200 and ra.json()["analyst_verdict"] == "malicious"


def test_verdict_admin_overrides_owner(tmp_path, monkeypatch):
    # L'admin passe outre le propriétaire (même mécanisme que DELETE /saved).
    c, q, tp = _client_forward_auth(tmp_path, monkeypatch, "alice")
    monkeypatch.setenv("OCULAR_ADMIN_TOKEN", "ADM")
    _seed_job(q, tp)
    sid = c.post("/saved", json={"job_id": "jx"}).json()["id"]

    cb = TestClient(app)
    cb.headers.update({"X-Forwarded-User": "bob", "X-Admin-Token": "ADM"})
    r = cb.post(f"/saved/{sid}/verdict", json={"analyst_verdict": "suspicious"})
    assert r.status_code == 200 and r.json()["analyst_verdict"] == "suspicious"


def test_verdict_owner_happy_path_plain_bearer_unchanged(tmp_path, monkeypatch):
    # Parcours mono-analyste (bearer partagé, saved_by="token") : le
    # propriétaire agit sur sa propre ligne, inchangé par le scoping.
    c, q, tp = _client(tmp_path, monkeypatch)
    _seed_job(q, tp)
    sid = c.post("/saved", json={"job_id": "jx"}).json()["id"]
    r = c.post(f"/saved/{sid}/verdict", json={"analyst_verdict": "legitimate"})
    assert r.status_code == 200 and r.json()["analyst"] == "token"
