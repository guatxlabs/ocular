...
def _fake_redis(keys=None):
    """Vraie fakeredis pré-seedée (plus de faux client Redis maison)."""
...
def test_sweep_orphans_removes_orphan_containers_only(monkeypatch):
    """Les trois tests de `sweep_orphans` renvoyaient tous un `docker ps` VIDE :
    le corps de boucle qui appelle `stop_session` sur un conteneur orphelin
    (celui qui a détruit une vraie session lors de la reproduction) n'était
    exercé par AUCUN test, et le garde-fou `startswith(_CONTAINER_PREFIX)`
    n'était vérifié que côté réseau."""
    from broker.sessions import sweep_orphans
    calls = []
    stopped = []
