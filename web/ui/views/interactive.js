...
export function renderInteractive(app) {
  ...
  function onPageHide() {
    ...
    try {
      // NB : `sendBeacon` émet un POST — or /sessions/{id} n'expose QUE DELETE.
      // Cet appel n'a donc AUCUN handler correspondant (il ne libère rien) : il
      // est purement décoratif, conservé pour le cas `fetch(keepalive)` ci-dessous
      // (navigateurs sans sendBeacon) qui, LUI, porte bien DELETE + Authorization.
      // Le vrai nettoyage reste le reaper/disconnect_grace (cf. commentaire ci-dessus).
