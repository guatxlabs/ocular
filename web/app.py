...
def _check_admin(request) -> JSONResponse | None:
    """Garde admin de `DELETE /saved*` (extraite de `_auth`) : autorise
...
# Plafond de taille de corps de requête (garde anti-OOM).
...
# web/middleware.py. Enregistré en DERNIER -> middleware utilisateur
...
# (allègement de `app.py`). Réimportés sous leurs noms `_préfixés` historiques pour ne
...
def _may_edit_saved(request: Request, meta: dict) -> bool:
    """True si l'appelant peut MUTER cette sauvegarde (verdict analyste).

    Miroir PARTIEL du modèle de session (`_owns_session`) : l'admin passe outre
    (même mécanisme que `DELETE /saved` : `X-Admin-Token` ou groupe admin IdP) ;
    sinon le propriétaire enregistré (`saved_by`) doit correspondre à l'identité
    de l'appelant (comparaison temps-constant).

    Deux écarts ASSUMÉS avec les sessions (documentés) :
    - la LECTURE reste GLOBALE (liste + résultat partagés). Il n'y a donc pas
      d'oracle d'existence à protéger côté lecture — tout le monde voit déjà
      toutes les lignes ; seule l'ÉCRITURE de verdict d'AUTRUI est restreinte.
    - une ligne SANS propriétaire enregistré (`saved_by` NULL/vide) reste
      éditable par tout appelant authentifié. C'est le cas du parcours
      mono-analyste (bearer partagé : `saved_by="token"` pour tous, donc ils
      partagent déjà le même principal) et des lignes HÉRITÉES d'avant la colonne
      `saved_by`. Fail-closer ici casserait le mono-analyste (aucun admin n'y est
      configuré — cf. tests) et l'accès aux lignes héritées, sans gain réel."""
    if _admin_granted(request):
        return True
    owner = meta.get("saved_by") or ""
    if not owner:
        return True
...
def set_saved_verdict(sid: int, body: AnalystVerdictRequest, request: Request) -> dict:
    # Route non-admin : un bearer/forward-auth normal suffit pour classer SA
    # PROPRE sauvegarde. Écraser le verdict d'une sauvegarde appartenant à un
    # AUTRE analyste exige d'en être le propriétaire (`saved_by`) OU l'admin ;
    # à défaut -> 404 (MÊME réponse que « inconnue » : pas d'oracle, miroir de
    # `_owned_session_or_404`). Seul DELETE /saved (flush) exige X-Admin-Token.
    ...
    with saved_conn() as conn:
        ...
        if meta is None or not _may_edit_saved(request, meta):
