...
PATTERNS: list[tuple[str, str, Severity]] = [
    ...
    (r"localStorage\.setItem\s*\(\s*[\"']([^\"']+)[\"']", "Local storage write", "medium"),
    (r"navigator\.userAgent", "Browser detection", "medium"),
    (r"navigator\.platform", "OS detection", "medium"),
    (r"screen\.width|screen\.height", "Resolution detection", "medium"),
    (r"navigator\.language", "Language detection", "low"),
    (r"on(?:click|load|error|focus|blur|submit)\s*=\s*[\"']([^\"']+)[\"']", "Event handler", "medium"),
    (r"addEventListener\s*\(\s*[\"']([^\"']+)[\"']", "Event listener", "medium"),
    (r"onsubmit\s*=", "Form submit handler", "critical"),
    (r"oncopy\s*=\s*[\"']return\s+false[\"']", "Copy disabled", "medium"),
    (r"onpaste\s*=", "Paste handler", "medium"),
    ...
    (r"confirm.*identity", "Identity confirmation text", "high"),
    (r"update.*payment", "Payment update text", "high"),
