...
class DnsResolutionError(ValueError):
    ...
    chercher une règle SSRF inexistante au lieu d'une panne DNS — d'où ce type
    distinct."""
...
def is_ip_allowed(ip: str | ipaddress.IPv4Address | ipaddress.IPv6Address) -> bool:
    ...
    # rejette explicitement : le multicast n'est pas couvert par `is_global`
    # seul — cohérent avec le docstring ci-dessus, qui annonçait déjà ce rejet.
