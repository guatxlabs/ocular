...
1. Créer `deploy/.env` (copie de `deploy/.env.example`) avec `OCULAR_TOKEN` **et** (si l'admin par
   token est voulu) `OCULAR_ADMIN_TOKEN`, tous deux générés par `openssl rand -base64 32` — jamais
