...
| `OCULAR_MAX_INTERNAL_CAPTURE_BYTES` | `134217728` (128 Mio) | Réponse `/capture` du `session_server` **refusée** -> `502`. Doit rester au-dessus du pire cas MESURÉ : **114,10 Mio** (résultat saturé d'octets nuls + 2 artefacts au plafond encodés en base64), soit 13,90 Mio de marge. |
...
**Ce qui est garanti, et par quelle mesure.** Après `ResultBuilder.build`, `len(json.dumps(result))` ≤ `OCULAR_MAX_RESULT_JSON_BYTES`, quoi qu'émette la page — vérifié sur trois entrées adverses (octets nuls en console, en `post_data`, et tous les champs au plafond) par `tests/test_result_size_limits_adverse.py`. Le maillon suivant est vérifié de la même façon : le pire cas du payload `/capture` complet (résultat + blobs base64) est **mesuré** à 114,10 Mio contre un plafond de lecture de 128 Mio. Le délestage ne touche jamais aux `screenshots`, au journal `dynamic_steps` (actions de l'analyste) ni au `triage`.
