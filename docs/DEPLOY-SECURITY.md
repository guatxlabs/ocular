...
| `OCULAR_MAX_TEXT_BYTES` | `32768` (32 Kio) | Budget par **défaut** d'un champ déclaré `CLIP` qui **ne nomme pas** le sien (`engine.result.Decl.budget`). Aucun champ du modèle n'est dans ce cas aujourd'hui : cette variable existe pour que le prochain champ texte dicté par la page soit borné **par sa seule déclaration**. Même échelle que les trois budgets calibrés ci-dessus, dont il reprend la valeur. |
...
Toute coupe de (b) est comptée dans `truncation.text_truncated` — nombre de **champs** coupés, pas d'entrées — **et nommée sur l'entrée elle-même** dans `truncated_fields` (`url`, `post_data`, `headers`, `text`, `title`, `final_url` aujourd'hui ; la liste est **dérivée** de `engine.result.CLIP_PLAN`, pas écrite dans le code de la coupe, et un champ `CLIP` ne peut vivre que sur un porteur capable de le nommer — le contraire est refusé à l'import). Le compteur global répond à « ce résultat est-il complet ? » ; il ne désigne aucune ligne. `post_data_truncated` reste servi pour les payloads déjà stockés, mais il est désormais **dérivé** de `truncated_fields`, réconcilié dans les deux sens par le modèle. L'UI affiche un badge « ✂ coupé » sur la ligne réseau et la ligne console concernées.

**(b quater) Le BALAYAGE a UNE seule place — mais ce n'est pas le seul travail lourd du tier, et le second ne se déporte pas.** Le balayage d'un document (`scan_html` + `extract_forms` + `extract_mailtos`) a **deux** appelants : l'analyse du panneau live et la composition du résultat de `/capture`. Seul le premier était borné. Mesuré (uvicorn + le vrai client interne et son échéance de 5,0 s, page de 512 Kio de contenu hostile, DOM mutant d'un octet par lecture) : **1** `/capture` concurrent -> polls `/live` servis **4/4** (pire latence 3 960 ms) ; **3** -> **4/4** (3 155 ms) ; **6** -> **0/4**, tous en `502` (5 225 ms). Désormais les deux appelants passent par la **même** place unique (`_bounded_scan` : une analyse à la fois, toujours hors de la boucle d'évènements) — mêmes mesures après : **4/4**, **4/4**, **4/4**, pire latence **205 / 160 / 187 ms**, donc indépendante du nombre de captures en vol. Un poll servi pendant une capture porte `analysis_stale: true` : la péremption est **dite**, jamais absorbée. La propriété est gardée par une garde statique dérivée d'`engine.static` (aucune fonction de balayage appelée hors des deux ouvriers purs, aucun ouvrier invoqué hors de la place unique), pas par relecture.

Le balayage n'est cependant **pas** le seul travail lourd du tier, et cette place unique ne le couvre **pas**. La composition de la réponse `/capture` — `base64` des blobs + `result.model_dump(mode="json")` (`wrapper_payload`), puis la sérialisation `json.dumps` de la réponse par Starlette — s'exécute **sur la boucle d'évènements**, APRÈS le balayage borné, et hors de la place unique. Mesuré au pire cas (résultat au plafond `32 Mio` + artefacts au plafond, payload **90,7 Mio**) : `base64`+`model_dump` **≈ 220 ms**, `json.dumps` **≈ 410 ms**, soit **≈ 650 ms de CPU sur la boucle par capture**. Effet mesuré sur `/live`, même banc, la seule variable étant OÙ pèse la page : page lourde en **motifs** (le poids est dans le balayage) -> `/live` pire latence **≈ 135 ms** ; page lourde en **blob** (le poids est APRÈS le balayage) -> `/live` pire latence **≈ 1 200 ms**, soit **×10**. Le service n'est jamais perdu par cette porte (les polls restent servis), mais la latence l'est.

**Pourquoi ce coût ne se déporte pas comme le balayage.** `run_in_threadpool` ne libère la boucle que pour du travail qui **relâche le GIL** — le balayage, une suite de petits appels Python entrelaçables, en profite. La composition n'en profite **pas** : `json.dumps` (encodeur C), `base64.b64encode` (C) et `model_dump` **tiennent le GIL** pendant toute leur durée. Mesuré : `json.dumps` d'un objet de 84 Mio en ligne bloque la boucle **≈ 350 ms** ; le MÊME appel via `run_in_threadpool` la bloque **≈ 356 ms** — aucun gain. Le même GIL est ce qui affame l'échantillonneur mémoire pendant `json.dumps`, noté en **(c)** ci-dessous. Déporter la composition serait donc un correctif **qui ne corrige rien** ; la vraie borne serait de ne pas matérialiser 90 Mio de JSON sur la boucle (transport binaire des blobs hors base64, ou encodeur incrémental qui rend la main) — un chantier de transport, pas un `await`. En l'état, c'est un coût **mesuré et documenté**, pas un coût nié.
...
| `OCULAR_MAX_LIVE_JSON_BYTES` | `8388608` (8 Mio) | Le `session_server` **mesure** sa réponse `/live` et réduit **toute fenêtre de volume variable** jusqu'à repasser dessous, en comptant tout dans `truncation`. **Trois natures**, les seules que JSON laisse varier avec ce que la page émet : une **chaîne** est coupée (`<champ>_chars_dropped`) ; un **tableau** — `list` OU `tuple` — perd des éléments (`<champ>_dropped`) ; un **objet** perd des paires, borné sur **clef ET valeur** comme `engine.wrapper._clip_mapping` (`<champ>_dropped`) — une clef dictée par la page ne peut donc pas rester dans la charpente et doubler la réponse. La réduction est un **partage max-min**, PAS un prorata uniforme : chaque fenêtre reçoit une **part** du budget, une fenêtre qui tient dans sa part garde tout et libère son reste, et seules celles qui débordent sont amputées — sinon la chaîne de 9 caractères dictée par le CODE (`verdict`) serait coupée dans la même proportion que la liste de plusieurs Mio dictée par la page. Borne haute `32 Mio`, **ramenée** sous le plafond de lecture si la configuration l'y fait passer (cf. propriété 3). |
...
**Le délestage ET la coupe sont DÉRIVÉS DU MODÈLE, plus jamais d'une liste écrite à la main.** Il portait sur trois noms (`network`, `console`, `static_findings`) et ratait `dom.forms` / `dom.mailtos`, que les quatre tiers remplissent **depuis le contenu de la page** (via `static.extract_forms`/`extract_mailtos`). Mesuré avec `OCULAR_MAX_RESULT_JSON_BYTES=262144` — valeur **dans** les bornes publiées, sur le **vrai chemin** page -> `extract_forms`/`extract_mailtos` -> `build` (une page de 120 formulaires + 120 `mailto:` : les plafonds d'extraction en retiennent **100** de chaque et tronquent chacun, `action[:500]`/`mailto[:320]`) : résultat de **491 245 o.** (×1,87 le plafond), `truncation` à **zéro**, donc annoncé **complet**, sans le moindre WARNING. Un résultat *composé à la main* avec 120 éléments bruts de 500 o. pèse **725 505 o.** (mesuré), mais **aucune page ne peut le produire** : c'est de là que venait le chiffre publié ici pendant un tour (`725 493 o.`), et il surestimait de 48 % l'ampleur atteignable depuis un contenu analysé. Désormais chaque champ du modèle porte une **déclaration de nature** (`engine.result.FIELD_VOLUME` : délesté / coupé / protégé / borné par construction / résiduel), et **les deux natures dictées par la page sont dérivées de cette table** : `SHED_PLAN` pour les listes à délester, `CLIP_PLAN` pour les champs à couper (avec le budget que la déclaration nomme). La coupe était encore énumérée à la main — six noms dans `ResultBuilder.build` — si bien que déclarer un septième champ `CLIP` **ne faisait rien** : mesuré, un `DomInfo.meta_description` (une ligne de modèle, une de déclaration) rempli de 200 000 caractères par la page sortait **entier**, `truncated_fields` vide, pour un résultat de **1 200 915 o.** sous un plafond de 262 144 ; sur un porteur délestable (`ConsoleEntry.stack`), **1 048 576 o.** étaient conservés pour un plafond par entrée de 32 768 avec `truncation` **vide**, donc annoncé complet. Trois refus **à l'import** ferment la classe : un champ de volume variable **non déclaré**, une déclaration que la garde ne sait pas appliquer à la **forme** du champ (`SHED` sur autre chose qu'une liste, `CLIP` sur autre chose qu'une chaîne ou un `dict[str, str]`, `CLIP` sur un porteur sans `truncated_fields`), et un **budget de coupe** que `engine.wrapper` n'implémente pas. Un champ ajouté demain ne peut plus être oublié : il est couvert, ou il ne s'exécute pas.
