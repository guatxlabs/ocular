...
- **Smoke-test e2e de l'UI.** Un smoke Playwright/Camoufox (l'image runner embarque déjà Camoufox) sur les 4 vues principales (login, submit, jobs, détail/verdict, interactif) : pose du token, chargement de la route, marqueur stable attendu (titre `h2`, présence du canvas noVNC…), zéro `pageerror`. Bornes : headless, réseau interne, aucun résidu de session. Hors matrice CI actuelle (pas de navigateur sur les runners GitHub) → cible `make` dédiée, comme `test-int`.
