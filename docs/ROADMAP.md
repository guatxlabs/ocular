...
Moteur autonome de **capture + analyse web durci** (fusion de trois outils internes antérieurs : capture, analyse HTML, bypass anti-bot). Dépôt autonome.

**Méthode** (éprouvée sur toutes les phases) : brainstorm → spec → plan → SDD (implémenteur + relecteur par tâche) → revue indépendante (archi / sécu / qualité) → **e2e réel Docker** → merge local. On ne merge jamais sans e2e réel : la boucle a attrapé à répétition de vrais défauts (Dockerfiles incomplets, timing-attack, TOCTOU, double-fault, réflexion 422, `/artifacts:ro`, sur-classification de verdict…).

**Contraintes permanentes** : périmètre = ce dépôt uniquement ; séparation de privilèges (web sans docker.sock → Redis → broker → runner éphémère durci) ; pas de fuite de secret (réseau/args/logs) ; portable (OIDC/LDAP/reverse-proxy quelconque, `.env` ou Vault/SOPS) ; DRY, pas de monolithe, pas de hardcode sécu.

Légende : ✅ fait & mergé · 🔜 à faire (priorisé) · ⏳ différé (dette identifiée) · 🧩 sous-projet.

---

## ✅ Finitions batch-3 (retour utilisateur 2026-07-16) — LIVRÉE
...
- **Crop bas/droite de la vue interactive (footer coupé sur certains sites)** — cause racine : le conteneur session n'a PAS de window manager, donc Firefox/Camoufox s'ouvrait plus grand que l'écran Xvfb (1280×720), ancré en haut-gauche → bas+droite hors framebuffer. Fix : `session_server._fit_browser_window()` force la fenêtre à couvrir exactement l'écran via xdotool (déjà présent, best-effort, plusieurs tentatives).
...
## ✅ Revue holistique (sécu + correctness + qualité) + remédiation (2026-07-16) — LIVRÉE

Deux passes de revue adversariale indépendante : (A) **sécu réseau/pivot** → `docs/DEPLOY-SECURITY.md` + durcissement (prefs navigateur QUIC/WebTransport/loopback/DNS-prefetch fermés `engine/browser_prefs`, mode strict `OCULAR_REQUIRE_EGRESS_GUARD` fail-closed + propagation runners, résiduels L3 documentés). (B) **holistique** (sécu backend, sécu frontend, correctness engine/runners, correctness web/broker/bus, qualité/archi). Suite **594 tests / 0 échec**. Remédiation des points confirmés :
...
## ✅ Revue holistique (sécu + correctness + qualité) — REMÉDIÉE (2026-07-18, 655 tests / 0 échec)

Relecture adversariale indépendante (read-only) de tout le dépôt, modèle de menace prod/entreprise. **0 Critical permettant exécution de code / évasion conteneur / accès non-auth.** Corrigés :
...
## ✅ Refactors qualité (dette relevée en revue de code) — LIVRÉE (2026-07-16, 594 tests / 0 échec)
...
## ✅ Couche IA/ML de triage + 2e avis + calibration — LIVRÉE (2026-07-18, 629 tests / 0 échec ; Docker 627+2 skip node)
...
**Sécu** (revue finale indépendante, 0 Critical/0 Important) : en configuration par défaut, **zéro nouvel egress, zéro nouveau chemin de privilège** ; séparation web→Redis→broker→runner inchangée.
...
| **Passe 2.5 — Utilisabilité + UI** | Artefacts sur volume disque (anti-traversal, DOM jamais inline), auth Bearer fail-closed temps-constant, **UI PWA vanilla-JS** (accent `#8b5cf6`, XSS-clean), Makefile + compose durci + GC + README. |
...
## ✅ Phase 3d — Correctness + UX + durcissement interactif — LIVRÉE (A–J tous mergés)

Regroupe le retour utilisateur (2026-07-13) + finitions. Chaque item passe par la méthode (spec courte → SDD → e2e).

> ⚠️ **Les specs A1 à J1 ci-dessous sont conservées AU PRÉSENT, telles qu'elles ont été écrites avant réalisation.** Elles décrivent le défaut et l'attendu, pas un reste à faire — c'est l'encadré d'état juste en dessous qui fait foi. Le titre de cette section portait encore « 🔜 » alors que son propre encadré disait « COMPLÈTE » : un lecteur ne pouvait pas savoir laquelle des deux affirmations valait.
...
> - **3d-2** : ✅ **I** filtrage SOC (`filter.js`, structuré, sans ReDoS) · ✅ **C** interactif (panneau live pollé/filtrable, fermeture auto onglet 60s + fermeture brutale `disconnected_at`/reaper grâce, sauvegarde) · ✅ **B** Turnstile (mapping viewport→écran `mozInnerScreen` + retry — **validé en direct sur un site protégé par Turnstile** (mapping viewport→écran vérifié)) · ✅ **J** recalibration verdict (re-tier + corroboration phishing/obfuscation — login légitime=benign, phishing/malware=malicious, faux négatifs audités+corrigés, EN+FR).
>
> **Suivis (dette, non bloquants)** : ~~(a) Turnstile — le retry ajoute ~4s à toute capture ; gater sur un indicateur Cloudflare (iframe `challenges.cloudflare.com`) pour ne payer le délai que quand un challenge existe.~~ → **✅ RÉSOLU (phase3f-F1a)** : `solve_turnstile` POLL l'indicateur DOM avant tout screenshot/detect/clic et rend `None` s'il est absent — 0 screenshot, 0 opencv sur une page sans Turnstile. Le poll est BORNÉ (6 × 0,6 s) et non one-shot, parce que Cloudflare injecte le widget de façon ASYNCHRONE : un check unique après `goto` le manquait et sautait la résolution à tort — régression corrigée et couverte par `test_solve_turnstile_cf_indicator_appears_late_runs_full_retry_loop`. **Résiduel assumé** : une page SANS Turnstile paie encore jusqu'à ~3,6 s de `page.evaluate` légers. Le gain de mur-à-mur est donc modeste ; celui de CPU ne l'est pas (plus de capture d'écran ni de template matching). Raccourcir la fenêtre rouvrirait la régression asynchrone. (b) Interactif — le poll `/live` ne réarme pas `mark_connected` (ok tant qu'il n'y a pas de reconnect auto RFB). ~~(c) Langage d'urgence phishing : EN+FR couverts ; autres langues rattrapées par le cluster form-externe mais patterns dédiés = dette.~~ → **✅ RÉSOLU** : EN/FR/**ES/DE/PT**, 12 patterns rejoignant le cluster `_URGENCY`, ReDoS-safe et anti-faux-positif (cf. § Différés techniques). Ce suivi contredisait sa propre résolution, écrite trente lignes plus bas.
...
- **Gating Turnstile** : détection vision (~4s) seulement si un indicateur Cloudflare est présent (poll de l'indicateur car injecté async — validé live : un site sans Turnstile n'engage aucune tentative, un site protégé est bien traité).
...
## ✅ Finitions UX + interactif + capture (retour utilisateur 2026-07-15) — LIVRÉE
...
- **URL en domaine nu / http / https** : champ UI `type="text"` (fini le rejet natif du domaine nu) ; normalisation serveur canonique (déjà correcte).
...
Historique scanné avant publication : `deploy/.env` **jamais** indexé, aucune clé
...
- ~~**Validation OIDC JWT in-app** (3e)~~ → **✅ FAIT** : `web/oidc.py` + `OCULAR_OIDC_ENABLED` (opt-in strict, OFF par défaut) valide un bearer JWT — signature RSA contre le JWKS de l'IdP, puis `iss`/`aud`/`exp`/`nbf` — pour un Keycloak/Authentik **sans** reverse-proxy. Le forward-auth continue de couvrir le cas proxifié, et l'ordre de préséance (bearer statique → forward-auth → OIDC) garantit que l'ajout ne change la réponse d'AUCUNE requête que les deux premières voies tranchaient déjà. `OCULAR_ADMIN_GROUP` fonctionne à l'identique via le claim de groupes (chemin pointé accepté : `realm_access.roles` pour Keycloak). **Fail-closed partout** : configuration incomplète, JWKS injoignable ou en clair sans opt-in, `kid` inconnu, `alg` hors {RS256,RS384,RS512}, module < 2048 bits, `exp` absent → refus. **Aucune dépendance ajoutée** — la vérification RSASSA-PKCS1-v1_5 est faite par reconstruction INTÉGRALE du bloc puis comparaison (rien n'est analysé, donc rien n'est toléré) ; la primitive est confrontée en test à des signatures produites par `cryptography`, et une forgerie à la Bleichenbacher sur `e=3` est fabriquée puis refusée. Couvert par `tests/test_oidc.py` (78 tests), dont la garantie de non-régression : opt-in éteint ⇒ `validate_bearer` n'est jamais appelée. **VÉRIFIÉ CONTRE UN IdP RÉEL** (2026-08-19, Keycloak 26.0 en conteneur, JWKS récupéré EN RÉSEAU et non injecté) : jeton d'un client muni d'un mappeur d'audience → **accepté**, identité extraite, `aud` MULTI-VALUÉ (`["ocular","master-realm","account"]`) correctement traité ; audience attendue différente → refusée ; issuer attendu différent → refusé. Le jeton `admin-cli` de Keycloak, lui, n'a AUCUN `aud` et est refusé — conforme à la RFC 9068, et documenté dans le README parce qu'un opérateur y butera. Les suites, elles, injectent le JWKS : c'est cette épreuve-ci qui prouve la compatibilité annoncée. **Résiduel assumé** : l'appel JWKS est synchrone sur un chemin `async` (au plus un par TTL de 300 s, échéance 5 s) ; PS256/ES256 non pris en charge (refus explicite, aucun des deux IdP ne les émet par défaut) ; Authentik non éprouvé.
...
- ~~**`/goto` initial peut échouer sur une session fraîche — `/health` readiness ≠ liveness (pré-existant 3b)**~~ → **✅ FAIT (2026-07-19)** : `/health` du `session_server` répondait `{"ok": true}` dès qu'uvicorn écoutait, alors que Camoufox n'était lancé que **paresseusement** au premier `/goto` (`_ensure_browser`). La disponibilité annoncée mesurait donc la **vivacité du process**, pas la **capacité à servir**. Devenu systématique avec `POST /sessions` asynchrone (202 + sonde `GET /sessions/{id}`) : la latence du POST synchrone (~7-9 s) masquait auparavant la fenêtre. Reproduit : `ready` à `t+3.5 s`, capture à `ready+0.0 s` → **502** (le conteneur répond 409 « no active session », rendu 502 par le web), capture à `ready+4.0 s` → 200. **Correctif** : le navigateur démarre désormais **à l'amorçage du conteneur** (tâche de fond de `_lifespan`, `_boot_browser`) et `/health` n'est vert **que si `_state["page"]` est vivante** — un état observé, **jamais une temporisation en dur** (le signal arrive quand la machine est prête, vite ou lentement). `_ensure_browser` est scindé (`_launch_browser` sous `_browser_lock` + re-vérification) : l'amorçage et un `/goto` concurrent ne lancent plus deux Camoufox / deux gardes egress. Échec de lancement → `boot_error`, `/health` reste 503 **fail-closed** : la sonde n'atteint jamais `ready`, l'échéance `session_ready_timeout` (30 s) expire et la session est stoppée. **Aucun changement côté web** : `GET /sessions/{id}` et `_wait_session_ready` partagent toujours la sonde `_session_state`, qui ne tient pour prête qu'une réponse 2xx — ils ne peuvent pas diverger. **Coût** : `ready` arrive à **~7,0-7,9 s** au lieu de ~3,5 s — c'est le délai honnête, l'ancien était le bug. **Preuve** : 5 sessions consécutives, capture lancée **immédiatement** après `ready` sans aucune temporisation → **5/5 HTTP 200**, `target` correctement navigué, zéro conteneur/réseau résiduel après suppression. Couvert par `tests/test_session_server_readiness.py` (6 tests) ; `tests/test_deploy_images.py::test_runner_recon_vnc_image_builds_and_smokes` devient un contrôle **fort** — il prouve que le navigateur démarre réellement sous durcissement complet (cap-drop ALL, seccomp, read-only, non-root, `--network none`), là où il ne prouvait que la présence d'un uvicorn à l'écoute.
  - **Résiduel accepté (mineur, non bloquant)** : au passage à `ready`, le thread d'amorçage `web._session_bootstrap` et un client qui capture aussitôt courent tous deux vers le conteneur. Si le client gagnait la course, il capturerait `about:blank` (`target` vide) au lieu de l'URL — HTTP 200, pas d'erreur, et jamais observé en pratique (le client sonde à 0,3 s d'intervalle, l'amorçage part sur la même observation avec plusieurs ms d'avance). Fermeture propre possible si le besoin apparaît : passer l'URL initiale au `session_server` au `docker run` pour qu'il navigue **avant** de se déclarer prêt — `ready` signifierait alors « page cible chargée », mais cela déplace la navigation initiale du web vers le broker (plomberie non triviale, hors périmètre de ce correctif).
- ~~**Langage d'urgence phishing multilingue**~~ → **✅ FAIT** : EN/FR/**ES/DE/PT** (12 patterns rejoignant le cluster `_URGENCY`, ReDoS-safe, anti-faux-positif). *(Autres langues au besoin.)*
- ~~**Bug `urlnorm` sur `data:`**~~ → **✅ FAIT** : `normalize_url` robuste (schemes non-réseau/malformé → rejet propre 400, plus de crash 500).
- **poll `/live` → `mark_connected`** (C) : uniquement si un reconnect auto RFB est ajouté un jour (sinon sans objet).
- ~~**Smoke-test e2e de l'UI**~~ → **✅ FAIT (2026-08-19)** : `tests/test_ui_smoke_e2e.py` (orchestration hôte) + `tests/smoke_ui/driver.py` (pilote Camoufox, exécuté DANS `ocular-runner-recon` — l'image tient déjà le couple Camoufox / `playwright==1.49.1` épinglé ensemble). Cible dédiée **`make smoke-ui`**. Marqueurs relevés dans le code des vues, jamais inventés : `h2` « Connexion » / « Mes analyses » / « Analyser une page » / « Session interactive », `.verdict-hero .verdict-badge` pour le détail, `.livestage .vncframe canvas` **plus** le statut `connecté` pour l'interactif — le `<canvas>` est créé par le constructeur `RFB` AVANT toute connexion, donc seul le statut prouve la poignée de main WebSocket de bout en bout. Le jeton est posé **par le formulaire** (donc vérifié serveur), et chaque route suivante est ouverte en **lien profond** (rechargement complet) : c'est ce qui prouve que le jeton et la liste locale survivent au `boot()`.
  - **Bornes tenues** : headless ; réseau **interne** — le conteneur de navigation partage la pile réseau du frontal (`--network container:…`) et vise `http://127.0.0.1:8000`, sans port publié ni cible tierce (bonus : origine `127.0.0.1` = contexte sécurisé, donc `crypto.subtle` de `api.js::sha256Hex` se comporte comme derrière TLS) ; durcissement identique au tier capture (cap-drop ALL, seccomp recon, read-only, non-root) ; **aucun résidu** — `--rm` + `docker rm -f` en `finally` (un dépassement de délai tue le CLI, pas le conteneur), et les conteneurs/réseaux de session sont comparés à un instantané pris avant, attendus disparus, balayés de force sinon.
  - **Garde-fou de sélection** : double marqueur `integration` + `smoke_ui`. C'est `integration` qui l'exclut de `pytest -m "not integration"` **même passé en ligne de commande** — comme le fait `deploy/Dockerfile.test` — puisqu'un `-m` en argument l'emporte sur celui d'`addopts` ; `smoke_ui` permet à `make test-int` de le laisser de côté. **MESURÉ** : `make test` → *1453 passed, 8 skipped, 21 deselected* (les 2 nouveaux tests désélectionnés, collecte intacte) ; pile down → *2 skipped* avec « pile Ocular non levée — lancer `make up` ».
  - **MESURÉ (pile levée)** : `make smoke-ui` → *2 passed en 43 s* ; vues seules 13,5 s, avec session interactive 28,5 s ; zéro `pageerror`, zéro erreur console ; verdict rendu `malicious` sur la charge `eval(atob(...))` ; session `sess-…` ouverte, canvas noVNC + flux connecté, refermée par l'UI, **zéro conteneur/réseau résiduel**.
  - **Le test MORD, prouvé** : un appel à une fonction inexistante posé dans `views/jobs.js` (image `web` rebuildée) fait échouer les deux tests en nommant la cause — `bite_test_regression_volontaire is not defined` — et l'assertion « zéro `pageerror` » part AVANT celle du marqueur. Ce contrôle a d'ailleurs révélé un défaut du pilote lui-même : la première version laissait l'exception remonter et fabriquait un verdict sans `page_errors`, donc rapportait « timeout en attendant `.emptyview` » **en taisant** l'exception JS déjà capturée. Le verdict est désormais construit dans tous les cas.
  - **Piège vérifié, à ne pas défaire** : le pilote n'utilise **ni `page.evaluate` ni `page.wait_for_function`**. La CSP de l'app shell (`script-src 'self'`, sans `'unsafe-eval'`) fait refuser par Firefox l'évaluation injectée par Playwright — d'où (1) un `pageerror` « call to eval() blocked by CSP » imputable à l'outillage, qui ferait échouer l'assertion sur une UI saine, et (2) une attente qui n'aboutit jamais alors que la condition est vraie (observé : « pas de bascule vers #/jobs — hash resté `'#/jobs'` »). Tout ce qui aurait demandé du JS passe côté Python : `page.url`, `page.text_content`, `page.request`. Désactiver la CSP pour faire marcher l'outillage aurait testé une autre application que celle qu'on déploie.

---

## 🧩 Sous-projets (hors phase 3)

- **Adaptateur SIEM** — intégration d'Ocular dans un SIEM interne (le tier analyse/capture surtout). **Ne pas modifier le SIEM ; construire l'adaptateur côté Ocular.**
- **Doc auth/secrets portable** — OIDC/LDAP/reverse-proxy quelconque, `.env` ou Vault/SOPS.

---

*Dernière mise à jour : 2026-08-19 — clôture du smoke e2e de l'UI (`make smoke-ui`), alignement du titre de la phase 3d sur son propre état, et clôture du suivi (c) que la section « Différés techniques » avait déjà résolu. Publication publique AGPL sur guatxlabs/ocular le 2026-07-19. Voir `docs/superpowers/specs/` et `docs/superpowers/plans/` pour le détail de chaque phase.*
