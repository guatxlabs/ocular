...
- **Publication publique** — ✅ **PUBLIÉ (2026-07-19)** sur `github.com/guatxlabs/ocular`, public, AGPL-3.0-or-later.
  - **Licence & conformité** : `COPYING` (AGPL v3 verbatim), SPDX sur les 120 fichiers du code propre (`vendor/` exclu — y déclarer l'AGPL serait une FAUSSE déclaration sur du MPL-2.0/MIT), `THIRD-PARTY-NOTICES.md` (noVNC MPL-2.0 + pako MIT, dont les fichiers de licence MANQUAIENT et ont été rapatriés de l'amont — la MPL §3.1 et la MIT exigent leur distribution). `pyproject` porte `license` + `license-files`. Job CI `licence` : refuse un `vendor/` sans sa licence ou un SPDX du projet posé sur du code tiers.
  - **Historique** : réécrit sous `GuatX <noreply@guatx.com>` (ni gmail, ni pseudo, ni nom légal), puis **intégralement signé** (clé SSH, 283 commits d'origine → Verified). Fait AVANT le premier remote — irréversible sinon. Scan complet : `deploy/.env` jamais indexé, aucune clé/jeton, chaînes suspectes = fixtures de `tests/`.
  - **CI** (`.github/workflows/ci.yml`) : 3 jobs (unitaire 3.11+3.13 + lint, dockerisée, conformité licence) + job de synthèse **au nom stable `CI`** (jamais les noms générés par matrice, qui rendraient `main` impoussable si la matrice change). `permissions: contents: read`, `persist-credentials: false`, actions épinglées au **SHA** (Dependabot les met à jour, SHA + commentaire de version).
  - **Protection `main`** (ruleset, bypass vide — vaut AUSSI contre l'auteur) : force-push bloqué, suppression bloquée, commits signés exigés, historique linéaire, check `CI` requis, PR obligatoire (0 approbation), résolution des conversations. **Prouvé par test** : push direct / force-push / suppression → refusés par le serveur, `main` intacte. Merge en **squash** uniquement.
  - **Sécurité GitHub** : secret scanning + push protection, CodeQL (actions/js-ts/python), private vulnerability reporting, Dependabot alertes + mises à jour (`.github/dependabot.yml`, pip/actions/docker ; **python 3.11-slim épinglé** — sauts majeurs/mineurs ignorés, la CI ne teste pas la version proposée). `GITHUB_TOKEN` en lecture seule, Actions sans création de PR.
  - **Vitrine** : description, `guatx.com`, 12 topics ; README avec **aperçu visuel** (`docs/images/` — verdict + score décomposé, DSL de soumission, détections, session live noVNC ; captures produites PAR Ocular lui-même). `CONTRIBUTING.md`, `SECURITY.md`, gabarits d'issue (redirection des failles) et de PR.
  - **Défaut corrigé en publiant** : `ic()`/`el({html})` posaient du SVG en `innerHTML` avec `cls` interpolé brut (CodeQL js/xss #6). Icônes converties en DONNÉES construites via `createElementNS` → **plus AUCUN `innerHTML` dans l'UI** (vendor exclu). Alertes CodeQL #1–#5 = faux positifs (assertions de test ; `REF_RE.fullmatch` ancré rend la traversée impossible).
  - **Piège rencontré (à rejouer sur core/forge)** : la règle *code scanning results* du ruleset bloque toute PR tant que CodeQL n'a pas produit ses résultats sur le commit de merge simulé — en *default setup* ce résultat manque, surtout après un changement de langages. Contournement sans toucher à la règle : pousser un commit (événement `synchronize`) pour re-scanner sur la config courante. À n'activer qu'APRÈS un scan vert sur `main`, comme la CI.
  - **Reste (non bloquant)** : doc auth/secrets portable (OIDC/LDAP/reverse-proxy, `.env`/Vault/SOPS) ; envisager de RETIRER la sous-règle *code scanning* du ruleset (a bloqué deux PR pour désalignement de config, jamais pour une vraie alerte ; le scan remonte de toute façon dans l'onglet Security).
- **Répliquer le modèle sur `core` et `forge`** — mêmes étapes, DANS L'ORDRE, et surtout : nettoyer l'identité + signer AVANT d'ajouter le remote GitHub (leur remote VPS existe déjà → réécrire fera diverger le bare, à réaligner). ⚠️ **Résidus connus** : `forge` tag `v0.0.1` et `core` branche `feat/ai-nl2soql` portent chacun un commit à la gmail hors de `main` (réécrire `main` ne les touche pas). CI à ADAPTER : `forge` = Python (pytest/ruff, comme Ocular), `core` = **Rust** (cargo test/clippy/fmt/audit). Le ruleset s'importe tel quel (JSON). **Ne rien faire sans feu vert explicite du propriétaire.**
