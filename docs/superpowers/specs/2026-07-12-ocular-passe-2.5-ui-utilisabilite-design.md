...
- **Pas d'UI** : seulement CLI/API. Or les deux outils internes d'origine avaient une interface web, et pour de l'analyse web une UI est nettement supérieure.
...
**But de la passe 2.5** : rendre le moteur d'analyse **réellement utilisable et déployable**, avec une UI web au **design du système interne de référence**.
...
- **UI web** vanilla-JS PWA reprenant le système de style interne (soumettre → jobs → détail).
...
| D4 | UI = **vanilla-JS PWA sur le système de style interne** (pas de framework, zéro build) | cohérence visuelle ET comportementale avec la stack, self-hosted, aligné « publiable » |
| D5 | Accent Ocular = **violet/indigo `#8b5cf6`** dans le système de style interne (dark navy `#0b0e14`, Inter+JetBrains Mono) | « même famille, app distincte » : chaque application a sa teinte (ici violet). Ajustable |
...
### 4.3 UI (vanilla-JS PWA sur le système de style interne)

Servie en statique par le tier web (FastAPI monte `web/ui/` sur `/`). Système repris du style interne de référence : dark navy, Inter+JetBrains Mono auto-hébergées (woff2), PWA (`sw.js` + `manifest.webmanifest`), i18n FR/EN, toggle thème clair/sombre. Accent violet `#8b5cf6`.
...
- **detail** — le cœur (façon sandbox + service de capture) : grand **screenshot** (via `GET /jobs/{id}/artifact/{ref}`), badge **verdict**, **findings static groupés par sévérité** (critical/high/medium/low, couleurs), **table réseau** (url/method/status/type/initiator), **console**, **DOM** (title/final_url/redirect_chain), lien téléchargement du DOM HTML.
...
3. **UI** : structure `web/ui/` (design de référence, PWA, i18n) + vues login/submit/jobs/detail + `api.js`. Servie par le web.
