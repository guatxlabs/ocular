...
- **Auteur** : guatx (+ Claude)
- **Sous-projet** : 1/3 (moteur autonome). Suivants : 2) adaptateur SIEM, 3) publication publique.

---

## 1. Contexte & problème

Ocular fusionne trois outils internes antérieurs (capture, analyse HTML, automatisation
navigateur furtive). Deux causes racines d'isolation y étaient structurelles et sont
supprimées par l'architecture ci-dessous, pas par des rustines : (1) le contenu hostile ne
doit jamais être rendu dans le navigateur de l'analyste ; (2) le socket Docker et le réseau
hôte n'ont rien à faire sur la surface exposée.
...
- **Schéma de résultat JSON stable** = contrat d'intégration future (SIEM interne, FAISS).

### Non-objectifs (explicitement hors v1 — YAGNI)
- Intégration SIEM/FAISS (sous-projet 2 ; seul le **contrat** est prévu ici).
...
- Refonte du module de vision interne (déjà porté dans `vision.py`).
...
| # | Décision | Rationale |
|---|---|---|
| D1 | Outil **autonome**, aucun couplage à l'écosystème interne ; intégration **SIEM** plus tard | un SIEM purple/blue est le bon consommateur. Inversion de dépendance via API+schéma. |
...
| D6 | Ancrer sur le socle d'automatisation navigateur interne (pas sur le service de capture d'écran) | ~80 % du moteur (stealth, vision, noVNC, capture réseau, intercept) déjà construit et fonctionnel. |
| D7 | **Pas** de contournement CDN générique ; stealth = Camoufox natif | ce type de bibliothèque est un résidu qui ne touche pas Turnstile. |
...
| Base | image dérivée du socle d'automatisation navigateur interne | image dérivée de l'analyseur HTML hostile interne |
...
Consommé plus tard par l'adaptateur SIEM et l'ingesteur FAISS (sous-projet 2). Un **test de
contrat** garantit que toute sortie valide ce schéma.

---

## 7. Migration (ce qui est repris des outils internes antérieurs)

| Apport repris | Détail |
|---|---|
| Socle d'automatisation navigateur furtive | **cœur** : Camoufox, vision (template-match) → clic OS, patch coreBundle, API riche (goto/screenshot/capture/intercept/vision-click-os), noVNC |
| Analyseur HTML hostile | détecteurs **static** (phishing/creds/obfuscation), parsing `.eml`, principe d'isolation |
| Service de capture d'écran | éventuellement le modèle prewarm/cache pour screenshot en volume |

Rien n'est repris de ce qui affaiblissait l'isolation (cf. §1).
...
ocular/
  ...
  runner-recon/        # image Camoufox + vision + noVNC (dérivée du socle interne)
  runner-analysis/     # image Chromium durci (dérivée de l'analyseur interne)
...
3. **Runner recon** : image dérivée du socle d'automatisation navigateur interne (Camoufox + vision + capture), profil capture, prewarm.
...
- **SIEM interne** : adaptateur consommant l'API + `result.schema.json` ; le moteur ne connaît pas son consommateur.
- **FAISS** : ingesteur des résultats (screenshots/DOM/findings vectorisés) pour recherche/corrélation.
- **Recon offensive** : le profil capture alimente le recon ; les findings peuvent référencer des jobs Ocular.
