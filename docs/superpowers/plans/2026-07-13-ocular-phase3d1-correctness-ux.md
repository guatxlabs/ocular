...
## Global Constraints
- Ne JAMAIS sortir de `ocular/` (périmètre = ce dépôt). Séparation de privilèges intacte. DRY, pas de monolithe, pas de hardcode sécu. TDD (test rouge d'abord). Chaque tâche = tests verts + `pytest -m "not integration" -q` sans régression.
