...
cp <source-interne>/vision.py runner_recon/vision.py
cp <source-interne>/turnstile_checkbox.png runner_recon/turnstile_checkbox.png
```
...
- [ ] **Step 4: Implémenter `runner_recon/capture.py`** (réutilise engine.result/static/verdict/urlnorm ; `capture_url` pilote Camoufox via l'API du socle interne + `vision`)
```python
from __future__ import annotations
...
- [ ] **Step 3: `runner_recon/Dockerfile`** (dérivé du socle d'automatisation navigateur interne, sans x11vnc/novnc)
```dockerfile
