...
## Global Constraints
- `ocular/` uniquement (périmètre = ce dépôt). Séparation de privilèges intacte (web sans docker.sock ; secret `X-Session-Secret` à la frontière conteneur, jamais loggé). DRY (réutilise `NetworkCapture`/`analyze_html`/`ResultBuilder`, `filter.js`). UI XSS-clean (el()/textNode). i18n FR→EN.
