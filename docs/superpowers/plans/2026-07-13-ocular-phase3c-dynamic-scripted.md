...
- Produces: sur stdin `{url, steps}` → le runner : `validate_steps` (défense en profondeur), `goto(url)`, arme NetworkCapture, `run_steps` (screenshots `capture` rangés comme blobs référencés dans le résultat), `emit_wrapper` d'un `OcularResult` avec un champ **`actions`** (le journal) et les screenshots labellisés. Sans steps sur stdin → comportement 3a **inchangé**.

**Détail d'implémentation** : repérer dans le runner recon existant (a) d'où vient l'URL (arg/env) et (b) l'appel `emit_wrapper`. Ajouter en tête : lire stdin non bloquant ; si JSON valide avec `steps`, basculer en mode scripté ; sinon 3a. Le `screenshot_cb(label)` prend `await page.screenshot()`, calcule `sha256_ref`, ajoute au dict de blobs + une entrée `{label, ref}` dans une liste `screenshots` du résultat. Le champ `actions` = journal de `run_steps`. Réutiliser `ResultBuilder` (ne pas refaire la structure du résultat à la main).
...
- Produces:
  ...
  - Le résultat renvoyé au client expose `actions` (journal) + screenshots labellisés (via le mécanisme d'artefacts existant, DOM jamais inline).
...
- Produces: un champ « script » (textarea JSON) sur le formulaire capture, avec 1–2 **exemples** insérables et un **retour de validation** (parse JSON client + message d'erreur serveur 422 affiché proprement) ; le rendu du résultat affiche le **journal d'actions** (index/verbe/ok/ms, valeurs déjà redigées côté serveur) et la **galerie de screenshots labellisés**. **XSS-clean** : tout texte via `textContent`/équivalent, jamais `innerHTML` de contenu non fiable (labels, erreurs, journal).
