...
## Global Constraints
- `ocular/` uniquement (périmètre = ce dépôt). Le verdict doit rester **explicable** (règles nommées, pas de boîte noire). Tests exhaustifs. Mettre à jour TOUS les tests existants qui dépendent de l'ancien comportement de verdict (leur intention change légitimement).
