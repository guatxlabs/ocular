...
## Global Constraints
- `ocular/` uniquement (périmètre = ce dépôt). Chemin capture 3a/3c préservé fonctionnellement. Pas de secret loggé. Tests + e2e réel avant merge.
...
```
Le div `.cf-turnstile`/`[data-sitekey]` et le script CF sont présents dans le HTML **avant** le rendu de l'iframe (validé sur un site protégé par Turnstile), donc l'indicateur est fiable dès après `goto`. Si l'indicateur est vrai → exécuter la boucle de retry existante (le widget async a le temps d'apparaître). Documente en commentaire. Ainsi une capture sans Turnstile (cas courant) ne subit plus les ~4s.
...
- [ ] **e2e réel** (rebuild `ocular-runner-recon`) : (a) capture `example.com` (sans CF) → **rapide** (pas de délai Turnstile ~4s ; mesurer la durée broker `runner done duration_ms`, doit chuter vs avant) ; (b) capture d'un site réellement protégé par Turnstile → **`turnstile_solved=True`** TOUJOURS (le gating ne l'a pas cassé) ; (c) corps chunked géant → 413.
