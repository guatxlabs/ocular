...
### Task 2: Détecteurs static (port depuis un détecteur interne antérieur)
...
- [ ] **Step 3: Implémenter `engine/static.py`** — porter les patterns réels du
détecteur interne antérieur. Table complète :
...
git commit -m "feat(engine): détecteurs static portés d'un détecteur interne antérieur"
```
