...
# présents dès `goto` — certains sites les injectent de façon ASYNCHRONE (script CF
...
async def solve_turnstile(
    ...
    **Gating (phase3f-F1a)** : avant tout screenshot/detect/clic, POLL
    ...
    courte fenêtre bornée — l'injection Cloudflare est ASYNCHRONE (un site protege par Turnstile),
...
async def _capture_dom(page: Any, url: str) -> tuple[bytes, str, str]:
    ...
    try/except) — un risque de dérive relevé en revue. Une seule
...
#
# WebRTC OFF : le garde egress est un proxy TCP HTTP/CONNECT ;
...
# lisibles.) Vérifié empiriquement : Turnstile reste résolu sur un site protégé et
...
async def _camoufox_session() -> AsyncIterator[Any]:
    ...
    **Fail-safe (décision tranchée)** : si le garde ne démarre pas
    ...
    # (source unique, partagée avec le tier interactif). Décidée AVANT l'import
    # Camoufox pour que le refus fail-closed soit immédiat.
...
async def capture_url(url: str, timeout_ms: int = 45000) -> tuple[OcularResult, dict[str, bytes]]:
    ...
    + clic OS xdotool (cf. runner_recon/vision.py, porté d'un socle
    d'automatisation navigateur interne), capture screenshots/réseau/DOM, puis
